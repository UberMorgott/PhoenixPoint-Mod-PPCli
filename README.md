...
`shots` counts pulls of the trigger, not projectiles: `PX_AssaultRifle` fires six per activation, and `projectilesPerShot` is reported alongside both counts. The strongest clean pass measured so far is 3 activations and 18 projectiles. `recovered` counts projectiles that another mod's exception left stuck in flight and that PPBridge had to release for the volley to continue; the exception is re-thrown unchanged, so it still reaches the log. A non-zero `recovered` **fails the run** and withholds the figures, because they would have been measured across a repair of the game; the count itself is still reported.
...
- A long volley against a target that dies can end in a named refusal rather than in figures. Once actors start dying, something throws inside `ProjectileLogic.OnTrajectoryEnd`; the stack is cut at the Harmony wrapper, so the throwing code cannot be identified. This is not a length ceiling — `shots` is accepted from 1 to 100 — but a run that asks for more shots than the target survives may refuse.
